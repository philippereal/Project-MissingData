...
* On marque les différentes catégories de données manquantes pour en garder la trace
```{r plotRes1, fig.height=10, fig.width=10, include=FALSE}
...
skin.only <- dataPima[is.na(dataPima$skin) & !is.na(dataPima$bp) & !is.na(dataPima$bmi),0]
skin.bp <-  dataPima[is.na(dataPima$skin) & is.na(dataPima$bp) & !is.na(dataPima$bmi),0]
skin.bmi <-  dataPima[is.na(dataPima$skin) & !is.na(dataPima$bp) & is.na(dataPima$bmi),0]
skin.bp.bmi<-  dataPima[is.na(dataPima$skin) & is.na(dataPima$bp) & is.na(dataPima$bmi),0]
bp.only<-  dataPima[!is.na(dataPima$skin) & is.na(dataPima$bp) & !is.na(dataPima$bmi),0]
bmi.only<-  dataPima[!is.na(dataPima$skin) & !is.na(dataPima$bp) & is.na(dataPima$bmi),0]
glu.only<-  dataPima[is.na(dataPima$glu),0]
...
skin.only <- dataPima[is.na(dataPima$skin) & !is.na(dataPima$bp) & !is.na(dataPima$bmi),0]
data[is.na(data$skin) & !is.na(data$bp) & !is.na(data$bmi),"missingCateg"]=1

skin.bp <-  dataPima[is.na(dataPima$skin) & is.na(dataPima$bp) & !is.na(dataPima$bmi),0]
data[is.na(dataPima$skin) & is.na(dataPima$bp) & !is.na(dataPima$bmi),"missingCateg"]=2

skin.bmi <-  dataPima[is.na(dataPima$skin) & !is.na(dataPima$bp) & is.na(dataPima$bmi),0]
data[is.na(dataPima$skin) & !is.na(dataPima$bp) & is.na(dataPima$bmi),"missingCateg"]=3

skin.bp.bmi<-  dataPima[is.na(dataPima$skin) & is.na(dataPima$bp) & is.na(dataPima$bmi),0]
data[is.na(dataPima$skin) & is.na(dataPima$bp) & is.na(dataPima$bmi),"missingCateg"]=4

bp.only<-  dataPima[!is.na(dataPima$skin) & is.na(dataPima$bp) & !is.na(dataPima$bmi),0]
data[!is.na(dataPima$skin) & is.na(dataPima$bp) & !is.na(dataPima$bmi),"missingCateg"]=5

bmi.only<-  dataPima[!is.na(dataPima$skin) & !is.na(dataPima$bp) & is.na(dataPima$bmi),0]
data[!is.na(dataPima$skin) & !is.na(dataPima$bp) & is.na(dataPima$bmi),"missingCateg"]=6

glu.only<-  dataPima[is.na(dataPima$glu),0]
data[is.na(data$glu),"missingCateg"]=7

data$missingCateg <- as.integer(data$missingCateg)

data$missingCateg <- factor(data$missingCateg,levels=c(0,1,2,3,4,5,6,7), labels=c("No-Missing","skin.only","skin.bp","skin.bmi","skin.bp.bmi","bp.only","bmi.only","glu.only"))


#data[,"missingCateg"]
dataMissing<-data
#dataMissing[,"missingCateg"]<-factor(dataMissing[,"missingCateg"])
...
#### 3.3 Imputation multiple autres méthodes - mice
...
(cette comparaison ne semble pas possible avec MIPCA)
```{r, echo=FALSE} 
...
Avec MIPCA on obtient 2 individus abérrants: le 685 et 454 qui ont des valeurs négatives pour skin et bmi
```{r, echo=FALSE} 
library(MASS)
...
#on concatene dataFramePima et la categorie de données manquantes: missingCateg
dataFramePima<- cbind(dataFramePima, dataMissing[,"missingCateg"])
names(dataFramePima)[ncol(dataFramePima)]<-"missingCateg"

#1 individus le 685 a skin<0 et  bmi <0 ? => pourquoi?
#1 individu le 454 a skin <0
#dataFramePima[dataFramePima$skin<0,]
#dataFramePima[dataFramePima$skin<7,]
#dataFramePima[dataFramePima$bmi<0,]
# => on les supprime
dataFramePima<-dataFramePima[-685,]
dataFramePima<-dataFramePima[-454,]
#on supprime aussi l'individus 295 avec skin = 2.071357 et 240  skin=4.701089
dataFramePima<-dataFramePima[-295,]
dataFramePima<-dataFramePima[-240,]


summary(dataPimaMASS)
