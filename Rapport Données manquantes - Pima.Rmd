...
insuline.miss<-sum(md.pattern(data[,2:6], plot =F)[, "Insuline"]== 0)
#print("Nombre de patterns si skin manquant")
skin.miss<-sum(md.pattern(data[,2:6], plot =F)[,"skin"]== 0)

print(paste0("Nombre de patterns si Insuline manquant :", insuline.miss))
print(paste0("Nombre de patterns si skin manquant :",skin.miss))
rm(aggr_plot)
...
nb.kfold<- estim_ncpPCA(data[,1:maxDim], ncp.min = 0, ncp.max = 6, method.cv = "Kfold", nbsim = 100, verbose = F)
plot(nb.kfold$criterion~names(nb.kfold$criterion),type="b",ylab="Critère MSEP", xlab="nombre de dimensions du dataset initial")
nb.kfold.new <- estim_ncpPCA(new.data[,1:maxDim], ncp.min = 0, ncp.max = 6, method.cv = "Kfold", nbsim = 100, verbose = F)
...
densityplot(conv.imp.mipca.boot.new, main="Densité MI avec mipca méthode=Regularized")
conv.imp.mipca.EM.boot.new <- prelim(imp.mipca.EM.boot.new, new.data[1:maxDim])
densityplot(conv.imp.mipca.EM.boot.new,main="Densité MI avec mipca méthode=EM")
```
...
```{r, sensibilite, fig.height=10, fig.width=10, message=F, warning=F}
delta <- c(0,10,100)
...
#age
diabete.insuline.noage <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~ npreg + glu + bp + skin + log.Insuline + bmi +  ped ,family=binomial(link="logit")))
mle.age<- pool.compare(diabete.insuline.glm, diabete.insuline.noage, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele complet vs modele sans la variable age : ", mle.age$pvalue))

#skin
diabete.insuline.noskin <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~ npreg + glu + bp + log.Insuline + bmi +  ped + age,family=binomial(link="logit")))
mle.skin<- pool.compare(diabete.insuline.glm, diabete.insuline.noskin, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele complet vs modele sans la variable skin : ", mle.skin$pvalue))

#log(Insuline) --> déja traité
diabete.insuline.noinsuline <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~ npreg + glu + bp + skin + bmi +  ped + age,family=binomial(link="logit")))
mle.noinsuline<- pool.compare(diabete.insuline.glm, diabete.insuline.noinsuline, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele complet vs modele sans la variable insuline : ", mle.noinsuline$pvalue))

#bp
diabete.insuline.nobp <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~ npreg + glu + skin + log.Insuline + bmi +  ped + age,family=binomial(link="logit")))
mle.bp<- pool.compare(diabete.insuline.glm, diabete.insuline.nobp, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele complet vs modele sans la variable bp : ", mle.bp$pvalue))

#bmi
diabete.insuline.nobmi <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~ npreg + glu + bp + skin + log.Insuline +  ped + age,family=binomial(link="logit")))
mle.bmi<- pool.compare(diabete.insuline.glm, diabete.insuline.nobmi, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele complet vs modele sans la variable bmi : ", mle.bmi$pvalue))

#glu
diabete.insuline.noglu <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~ npreg  + bp + skin + log.Insuline + bmi + ped + age,family=binomial(link="logit")))
mle.glu<- pool.compare(diabete.insuline.glm, diabete.insuline.noglu, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele complet vs modele sans la variable glu  : ", mle.glu$pvalue))

#npreg
diabete.insuline.nopreg <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~ bp + skin + glu+ log.Insuline + bmi + ped + age,family=binomial(link="logit")))
mle.npreg<- pool.compare(diabete.insuline.glm, diabete.insuline.nopreg, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele complet vs modele sans la variable npreg : ", mle.npreg$pvalue))

#npreg
diabete.insuline.noped <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~ bp + skin + glu+ log.Insuline + bmi + npreg + age,family=binomial(link="logit")))
mle.npreg<- pool.compare(diabete.insuline.glm, diabete.insuline.noped, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele complet vs modele sans la variable ped : ", mle.npreg$pvalue))

```

Toutes les variables semblent significatives, c'est le modèle complet qui a le meilleur AIC.
A partir de ce AIC moyen on pourrait essayer de supprimer les variables : bmi, age et bp.

```{r, echo = FALSE}
############### AIC MOYEN EXTRAIT DE ANALYSES##############################

AIC<-mean(sapply(diabete.insuline.glm$analyses, AIC))
print(paste0("AIC du modele complet : ",AIC))

AIC.noage<-mean(sapply(diabete.insuline.noage$analyses, AIC))
print(paste0("AIC du modele sans la variable age : ",AIC.noage))

AIC.noskin<-mean(sapply(diabete.insuline.noskin$analyses, AIC))
print(paste0("AIC du modele sans la variable skin : ", AIC.noskin))

AIC.noinsuline<-mean(sapply(diabete.insuline.noinsuline$analyses, AIC))
print(paste0("AIC du modele sans la variable insuline : ", AIC.noinsuline))

AIC.nobp<-mean(sapply(diabete.insuline.nobp$analyses, AIC))
print(paste0("AIC du modele sans la variable bp : ", AIC.nobp))

AIC.nobmi<-mean(sapply(diabete.insuline.nobmi$analyses, AIC))
print(paste0("AIC du modele sans la variable bmi : ", AIC.nobmi))

AIC.nopreg<-mean(sapply(diabete.insuline.nopreg$analyses, AIC))
print(paste0("AIC du modele sans la variable npreg : ", AIC.nopreg))

AIC.noglu<-mean(sapply(diabete.insuline.noglu$analyses, AIC))
print(paste0("AIC du modele sans la variable glu : ", AIC.noglu))

AIC.noped<-mean(sapply(diabete.insuline.noped$analyses, AIC))
print(paste0("AIC du modele sans la variable ped : ", AIC.noped))

```

On regarde maintenant et l'on compare certains modèles emboîtés en supprimant successivement les variables : age, bmi et bp.
Le meilleur modèle qui minimise le critère BIC est le modèle diabete.insuline.glm sans les variables :  bmi, age et bp
Soit en ne conservant que les 5 variables: npreg, ped, glu, skin et log.Insuline 
C'est aussi confirmé par les différents MLE réalisés sur les modèles emboîtés. 
Pour argumenter ce choix on peut se référer à l'annexe 2 - choix de modèle 


```{r AIC_Moyen,  echo = FALSE}

#Au final, on teste le modèle sans les variables bmi, glu et preg
AIC.complet<-mean(sapply(diabete.insuline.glm$analyses, AIC))
BIC.complet<-mean(sapply(diabete.insuline.glm$analyses, BIC))

diabete.insuline.glm7 <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~  npreg + glu + skin + log.Insuline + ped + bp + bmi  ,family=binomial(link="logit")))
AIC.7<-mean(sapply(diabete.insuline.glm7$analyses, AIC))
BIC.7<-mean(sapply(diabete.insuline.glm7$analyses, BIC))

diabete.insuline.nobmi <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~  npreg + glu + skin + log.Insuline + ped + bp + age  ,family=binomial(link="logit")))
AIC.7.nobmi<-mean(sapply(diabete.insuline.glm7$analyses, AIC))
AIC.7.nobmi<-mean(sapply(diabete.insuline.glm7$analyses, BIC))

diabete.insuline.glm6 <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~  npreg + glu + skin + log.Insuline + ped + bmi  ,family=binomial(link="logit")))
AIC.6<-mean(sapply(diabete.insuline.glm6$analyses, AIC))
BIC.6<-mean(sapply(diabete.insuline.glm6$analyses, BIC))

diabete.insuline.glm5 <- with(data=conv.imp.mipca.EM.boot.new ,exp=glm(type ~ npreg + glu + skin + log.Insuline + ped ,family=binomial(link="logit")))
...
AIC.5<-mean(sapply(diabete.insuline.glm5$analyses, AIC))
BIC.5<-mean(sapply(diabete.insuline.glm5$analyses, BIC))

mle.final<- pool.compare(diabete.insuline.glm, diabete.insuline.glm7, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele complet vs modele sans la variable age : ", mle.final$pvalue))
mle.final<- pool.compare(diabete.insuline.glm7, diabete.insuline.glm6, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele sans la variable age vs modele sans la variable age et bp: ", mle.final$pvalue))
mle.final<- pool.compare(diabete.insuline.glm6, diabete.insuline.glm5, method = "likelihood")
print(paste0("Test du rapport de vraisemblance modele sans la variable age et bp vs modele sans la variable age, bp et bmi: ", mle.final$pvalue))

print(paste0("AIC du modele complet : ", AIC.complet))
print(paste0("AIC du modele sans la variable age : ", AIC.7))
print(paste0("AIC du modele sans la variable age et bp : ", AIC.6))
print(paste0("AIC du modele sans la variable age, bp et bmi: ", AIC.5))

print(paste0("BIC du modele complet : ", BIC.complet))
print(paste0("BIC du modele sans la variable age : ", BIC.7))
print(paste0("BIC du modele sans la variable age et bp : ", BIC.6))
print(paste0("BIC du modele sans la variable age, bp et bmi: ", BIC.5))
...
Au final, les estimations des coefficients sont du même ordre de grandeur en comparaison de la méthode des cas concrets (= listwise deletion).
Mais de manière général sont plus faibles. Et en particulier biensûr pour les variables à fortes données manquantes: insuline et skin.  

```{r, logit.glm5-NA, warning=F, message=F,echo=FALSE}
diabete.listwisedeletion.glm5 <- glm(type ~ npreg + glu + skin + I(log(Insuline)) + ped, data=data, family = binomial(link="logit"))
...
```{r include=TRUE}
diabete.listwisedeletion.glm1 <- stepAIC(glm(type ~ npreg + glu + bp + skin + I(log(Insuline)) + bmi+ ped + age, data=data[ok,], family = binomial(link="logit")), direction="both", trace = F)
...
```{r, traite.reasMeanPCA, warning=F, message=F,eval=FALSE, echo=FALSE}

res.meanMipca<-lapply(imp.mipca.EM.boot.new$res.MI,colMeans)

res.meanMipca <- cbind(res.imputePCA[,],data[,10]) 
...
choix_modele<-regsubsets(type ~ . ,int=T, nbest=1,nvmax=4,method="exhaustive",data=res.imputePCA[,1:maxDim])
...
```{r include=FALSE}
library(MASS)
modele_step_BwdFwd_AIC <- step(m_sature, data=res.imputePCA[,1:maxDim], direction="both")
```

* Critère BIC
```{r include=FALSE}
K<-log(dim(res.imputePCA)[1])
m_StepBwdFwd_BIC <- step(m_sature, data=res.imputePCA[,1:maxDim], direction="both",k=K)
#help("step")
```
<div style="page-break-after: always;"></div>
...
res.pca<-PCA(res.imputePCA[,1:10],graph=FALSE, quali.sup = 9:10)
#summary(res.pca) 
...
```{r PCA0101, echo=FALSE, fig.width=12}
...
### Annexe 4 : Comparaison des corrélations des données complétés obtenues avec mice et missMA 
...
#data(Pima.te)
dataPimaMASS <- rbind(Pima.tr,Pima.te)
DataMat.mass=data.matrix(dataPimaMASS)
#mcor.mass<-cor(DataMat.mass)
#corrplot(mcor.mass, type="upper",  tl.col="black", tl.srt=8)

DataMat=data.matrix(res.imputePCA[,1:maxDim])
mcor<-cor(DataMat)
corrplot(mcor, type="upper",tl.col="black", tl.srt=8)

DataMat_mice=data.matrix(completedMice[,1:maxDim])
mcor_mice<-cor(DataMat_mice)
corrplot(mcor_mice, type="upper",tl.col="black", tl.srt=8)
```
```{r echo=FALSE, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
pairs(res.imputePCA[,1:maxDim], pch ='.')

```
```{r echo=FALSE, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
pairs(completedMice[,1:maxDim], pch ='.')
